...
function validate_config(package_json,callback) {
    ...
    if (o) {
        ...
        var host = process.env.npm_config_node_pre_gyp_mapnik_host || o.host;
        var protocol = url.parse(host).protocol;
        if (protocol === 'http:') {
...
module.exports.evaluate = function(package_json,options) {
    ...
    var host = process.env.npm_config_node_pre_gyp_mapnik_host || package_json.binary.host;
    opts.host = add_trailing_slash(eval_template(host,opts));
